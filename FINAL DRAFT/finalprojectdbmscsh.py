...
class StudentPage(QWidget):
    ...
    def login(self):
        ...
        try:
            ...
            cursor.execute("""
                SELECT TOP 1 sv_id, time_out
                FROM csh 
                WHERE sv_id = ? AND time_out IS NULL
                ORDER BY time_in DESC
            """, sv_id)
            closing_session = cursor.fetchone()

            if active_session:
                time_in = datetime.now()
                cursor.execute("""
                       UPDATE csh SET time_in = ? 
                       WHERE sv_id = ? AND time_out IS NULL
                       """, (time_in, sv_id))
                conn.commit()
            ...
            if closing_session:
                sv_id, time_out = closing_session
...
class CounselorMenu(QWidget):
    def __init__(self, stacked_widget, departments):
        ...
        layout.addWidget(QLabel(f"Counselor  - Programs: {', '.join(self.departments)}"))
...
class ManagementPage(QWidget):
    ...
    # Execute query
        ...
        # Special logic for student_violations
            if self.table_name == "student_violations":
                ...
                first_hours = get_hours(values.get("first_offense_id"))
                second_hours = get_hours(values.get("second_offense_id"))
                third_hours = get_hours(values.get("third_offense_id"))

                total_hours = first_hours + second_hours + third_hours
                values["remaining_hours"] = total_hours
